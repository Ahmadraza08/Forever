...
const PlaceOrder = () => {
  ...
  return (
    <div className='flex grid-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t'>
      {/* left Side */}
      <div className='flex flex-col gap-4 wfu;; *:selection:max-w-[480px]'>

        <div className='text-xl sm:text-2xl my-3'>
          <Title text1={'DILEVERY'} text2={'INFORMATION'}/>
        </div>
      ...
      <div className='mt-8'>
        ...
        <div className='mt-12'>
          ...
          <div className='flex gap-3 flex-col lg:flex-row'>
            <div onClick={()=>setMethod('stripe')} className='flex items-center gap-3 backdrop : border border-gray-300 p-2 px-3 cursor-pointer'>
              <p className={`min-w-3.5 h-3.5 border border-gray-300 rounded-full ${method === 'stripe' ? 'bg-green-400' : ''}`}></p>
              <img src={assets.stripe_logo} className='h-5 mx-4' alt="" />
            </div>

            <div onClick={()=>setMethod('razorpay')} className='flex items-center gap-3 backdrop : border border-gray-300 p-2 px-3 cursor-pointer'>
              <p className={`min-w-3.5 h-3.5 border border-gray-300 rounded-full ${method === 'razorpay' ? 'bg-green-400' : ''}`}></p>
              <img src={assets.razorpay_logo} className='h-5 mx-4' alt="" />
            </div>

            <div onClick={()=>setMethod('cod')} className='flex items-center gap-3 backdrop : border border-gray-300 p-2 px-3 cursor-pointer'>
